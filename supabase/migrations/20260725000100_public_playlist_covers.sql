-- Expose owner id and cover art so browsed public playlists render like the owner's own library.
drop function if exists public.get_public_playlists(uuid, integer);

create function public.get_public_playlists(p_owner_id uuid, p_limit integer default 20)
returns table (
  id uuid,
  owner_id uuid,
  title text,
...
as $$
  select pl.id, pl.owner_id, pl.title, pl.description, p.display_name, pl.cover_image_url,
